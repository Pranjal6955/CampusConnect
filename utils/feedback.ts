...
export async function canSubmitFeedback(
  ...
  try {
    ...
    if (!attended) {
      return {
        canSubmit: false,
        reason: 'You must attend the event to provide feedback',
      };
    }
  ...
  try {
    ...
    for (const docSnapshot of querySnapshot.docs) {
      ...
      try {
        ...
        console.error(
          `Error loading student data for feedback ${docSnapshot.id}:`,
          error
        );
...
export async function getOrganizerEventsWithFeedback(
  organizerId: string
): Promise<(Event & { feedbackCount: number; averageRating: number })[]> {
  try {
    ...
    const eventsWithFeedback: (Event & {
