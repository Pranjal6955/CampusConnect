  ...
  return (
    ...
    >
      <View className={`flex-1 ${isDark ? 'bg-black' : 'bg-gray-50'}`}>
        ...
        >
          ...
          <View className="px-6 py-6">
            ...
            <View className="mb-6">
              ...
              <Text
                className={`text-3xl font-extrabold leading-tight ${isDark ? 'text-white' : 'text-gray-900'}`}
              >
                {event.title}
              </Text>
            </View>
            ...
            <View className="flex-row justify-between mb-8">
              <View
                className={`flex-1 p-4 rounded-2xl mr-2 items-center justify-center ${isDark ? 'bg-gray-900' : 'bg-white'}`}
                style={{
                  shadowColor: '#000',
              ...
              >
                ...
                <Text
                  className={`text-xs font-medium ${isDark ? 'text-gray-400' : 'text-gray-500'} uppercase`}
                >
                  {t('events.going')}
                </Text>
              </View>
              <View
                className={`flex-1 p-4 rounded-2xl ml-2 items-center justify-center ${isDark ? 'bg-gray-900' : 'bg-white'}`}
                style={{
                  shadowColor: '#000',
                  shadowOffset: { width: 0, height: 2 },
                  shadowOpacity: 0.05,
            ...
            <View className="mb-8 space-y-4">
              ...
              <View className="flex-row items-start">
                <View
                  className={`w-10 h-10 rounded-full items-center justify-center mr-4 ${isDark ? 'bg-gray-800' : 'bg-blue-50'}`}
                >
              ...
              {!event.fullDayEvent && (
                <View className="flex-row items-start mt-4">
                  <View
                    className={`w-10 h-10 rounded-full items-center justify-center mr-4 ${isDark ? 'bg-gray-800' : 'bg-purple-50'}`}
                  >
              ...
              <View className="flex-row items-start mt-4">
                <View
                  className={`w-10 h-10 rounded-full items-center justify-center mr-4 ${isDark ? 'bg-gray-800' : 'bg-green-50'}`}
                >
        ...
        {/* Floating Action Bar */}
        <View
          className={`absolute bottom-0 left-0 right-0 p-6 pt-4 border-t ${isDark ? 'bg-black border-gray-800' : 'bg-white border-gray-100'}`}
        >
