  ...
  const handleSubmit = async () => {
    if (rating === 0) {
      Alert.alert(
        ...
        t('feedback.ratingRequiredMessage')
      );
      return;
    }
    ...
    if (comment.trim().length === 0) {
      Alert.alert(
        t('feedback.commentRequired'),
        t('feedback.commentRequiredMessage')
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        t('common.error'),
        error.message || t('feedback.failedToSubmit')
      );
    } finally {
      setSubmitting(false);
    }
    ...
    >
      <View className="flex-1 justify-end">
        ...
        >
          ...
          {/* Header */}
          <View
            className={`px-5 pb-4 ${isDark ? 'border-b border-gray-800' : 'border-b border-gray-200'}`}
          >
          ...
          >
            ...
            <View className="mb-6">
              ...
              {rating > 0 && (
                <Text
                  className={`text-center mt-3 text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}
                >
            ...
            >
              <Text
                className={`font-semibold text-base ${isDark ? 'text-gray-400' : 'text-gray-600'}`}
              >
                {t('common.cancel')}
              </Text>
            </TouchableOpacity>
